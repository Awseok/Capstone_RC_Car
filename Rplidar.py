...
class ReadRplidar(object):
    ...
    def process_data(self, data):
        max_distance = 0
        centerX = centerY = (int)(self.size / 2)
        m = DstarAL.Map(self.size, self.size)
        start = m.map[centerX][centerY]
        end = m.map[self.size -1][self.size -1]
        ...
        for angle in range(360):
            ...
            if distance > 0:                  
                ...
                m.set_obstacle([((int)(centerX - x), (int)(centerY + y))])
           
                if angle == 359:
                    dstar = DstarAL.Dstar(m)
                    dstar.run(start, end)
                    m.print_map()
                    m.clear()
